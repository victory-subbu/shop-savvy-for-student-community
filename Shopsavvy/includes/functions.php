...
function drawHeader($s=0){
	...
	<header class='header'>
    <div class='container'>
        <a href='index.php' ><img href='index.php' src='assets/img/ShopSavvy_logo.jpg'></a>";
		if(isLogged() == 1){
			logout();
			myaccount();
			echoaddadslog($s);
...
function echoaddadslog($s){
	...
	<a   class='btn btn-lg btn-block addads ' href='createads.php'>Sell item</a></div>";
	}else {
	echo "</header>";
	}
}
...
function echoaddadsnolog(){
	echo "</header>
	<div class='container control'>
	<a   class='btn btn-lg btn-block addads ' href='' data-toggle='modal' data-target='#myModal'>Item Details</a>
	</div>
	";
}
...
function myaccount(){
	...
	$ab=$_SESSION['usernamelogin'];
	$_SESSION['varname1']=$ab;
...
function getCityAreas($cityID){
    $areas=[];
    global $clink;
	$result=mysqli_query($clink,"SELECT areaID, areaName FROM areas");
    while($row=mysqli_fetch_row($result))
    {
        $areas[]=$row;
    }
